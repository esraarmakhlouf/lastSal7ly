...
namespace sal7lyAdmin.Controllers
{
    ...
    public class JobTitleController : Controller
    {
        ...
        public JobTitleController(IUnitOfWork uow, ISecurity security) { _uow = uow; _security = security; }

        public IActionResult Index()
        {
            //Check User Permission For this Page
            if (!UserAccountMannager.HasPermission(_uow, _security, AppSession.CurrentUser.Id, EN_Screens.JobTitle, EN_Permissions.View))
            { return Redirect("/Home"); }

            var result = _uow.JobTitlesRepository.GetAll().OrderByDescending(ent => ent.CreationDate).ToHashSet();
            return View(result);
        }
        ...
        {
            var result = _uow.JobTitlesRepository.GetAll().ToHashSet();
        ...
        public IActionResult OpenForm(long? id)
        {
            ...
            {
                //Check User Permission For this Page
                if (!UserAccountMannager.HasPermission(_uow, _security, AppSession.CurrentUser.Id, EN_Screens.JobTitle, EN_Permissions.Edit))
                { return Redirect("/Home"); }

                entity = _uow.JobTitlesRepository.GetById(id);
            ...
            {
                //Check User Permission For this Page
                if (!UserAccountMannager.HasPermission(_uow, _security, AppSession.CurrentUser.Id, EN_Screens.JobTitle, EN_Permissions.Create))
                { return Redirect("/Home"); }
            }
        ...
        [HttpPost]
        public IActionResult Save(JobTitle entity)
        {
            ...
            try
            {
                ...
                {
                    entity.Code = UIHelper.GeneratCode(EN_Screens.JobTitle, _uow);
                    ...
                    { _uow.JobTitlesRepository.Add(entity); }
                }
                else
                {
                    ...
                    TryValidateModel(entity);
                    if (ModelState.IsValid)
                    { _uow.JobTitlesRepository.Update(entity); }
                }
                _uow.Save();
        ...
        {
            //Check User Permission For this Page
            if (!UserAccountMannager.HasPermission(_uow, _security, AppSession.CurrentUser.Id, EN_Screens.JobTitle, EN_Permissions.Delete))
            { return Redirect("/Home"); }
            ...
            string errorMessage = "";
                ...
                try
                {
                    var dbObj = _uow.JobTitlesRepository.GetById(id);
                    if (dbObj == null)
                    {
                        status = "error";
                    ...
                    else
                    {
                        ...
                        _uow.JobTitlesRepository.Update(dbObj);
                        _uow.Save();
                    }
                }
                catch (Exception ex)
